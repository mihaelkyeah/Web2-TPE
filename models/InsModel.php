...
class InsModel extends Model {
    ...
    public function saveIns($name, $price, $details, $category) {
        $query = $this->getDb()->prepare('INSERT INTO `instrument` (`ins_name`, `price`, `ins_desc`, `id_categ_fk`) VALUES (?, ?, ?, ?)');
    ...
    public function updateIns($name, $price, $details, $category, $id) {
        $query = $this->getDb()->prepare('UPDATE `instrument` SET `ins_name` = ?, `price` = ?, `ins_desc` = ?, `id_categ_fk` = ? WHERE `instrument`.`id_instrument` = ?');
